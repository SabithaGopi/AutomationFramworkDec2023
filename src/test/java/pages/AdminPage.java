...
public class AdminPage extends Base {
	...
	
	{
		...
		logger.info("Admion Page display check");
		boolean b=false;
		try {
		...
		return b;		
	     ...
	     public boolean verifyTheUser(String name, String role) throws InterruptedException
	     {
	    	 ...
	    	 boolean b=false;
	    	 //int row=dr.findElement(By.xpath("//div[@role='table']/div[@class='oxd-table-body']/div/div")).get(0))

	    	   int row = dr.findElements(By.xpath("//div[@role='table]/div[@class='oxd-table-body']/div/div")).size();
	    	 logger.info("Total Rows:"+ row);
	    	 		
	    			 int userCol=2;
	    	  logger.info("UserCol"+ userCol);
	    	  int roleCol=3;
	    	  
	    	  logger.info("RoleCol"+ roleCol);
